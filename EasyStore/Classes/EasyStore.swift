...
open class EasyStore<T: Object> {
    ...
    open func postNotification(withUpdateType type: UpdateType) {
        NotificationCenter.default.post(Notification(name: Notification.Name(rawValue: notificationName()), object: nil, userInfo: [updateTypeKey(): type]))
    }
    ...
    open func observe(_ handler: @escaping (UpdateType) -> Void) -> NSObjectProtocol {
        return NotificationCenter.default.addObserver(forName: NSNotification.Name(rawValue: notificationName()), object: nil, queue: nil) {
